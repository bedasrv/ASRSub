...
LANG_NORM = {"eng": "en", "enm": "en", "en": "en", "jpn": "ja", "jp": "ja", "ja": "ja", "ind": "id", "id": "id"}

def _normalize_lang(value):

    key = re.sub(r"[^a-z0-9]+", "", str(value or "").strip().lower())
    return LANG_NORM.get(key, key)

def _registry_source_language(source, target_lang):
    """Map registry source aliases separately from target language."""
    source_lang = _normalize_lang(source)
    if source_lang == "ja":
        return "ja"
    if source_lang == "en":
        return "en"
    if source_lang == "id":
        return "id"
    if source_lang == "asr":
        return _normalize_lang(target_lang)
    return _normalize_lang(target_lang)
...
@contextmanager
def _registry_read_lock(path):
    """Read under the daemon's directory lock, plus its sidecar when present.

    Missing registries and sidecars are never created by GET requests; the
    existing parent directory provides the first-writer synchronization point.
    """
    ...
    try:
        ...
        fcntl.flock(directory_handle, fcntl.LOCK_SH)
...
def _has_ai_marker(path):
    """Require the ASR ownership marker in the first subtitle cue."""
    try:
        with open(path, encoding="utf-8", errors="replace") as fh:
            first = re.split(r"\r?\n[ \t]*\r?\n", fh.read().strip(), maxsplit=1)[0]
    except OSError:
        return False
    return "AI-generated by ASRSub" in first
...
class ControlAPIv2:
    ...
    def _load_control_key(self):
        """Secret file first, env fallback for tests (no secret values logged)."""
        candidates = []
        ...
        for p in uniq:
            try:
                ...
                continue
        return self._env().get("CONTROL_API_KEY", "").strip()

    def control_token(self):
        return self._load_control_key()
                ...
                for line in fh:
                    ...
                    if isinstance(e, dict):
                        ...
                        if e.get("language") is not None:
                            e["language"] = _normalize_lang(e.get("language"))
                        entries.append(e)
        except OSError:
            pass
        latest = {}
    ...
    def _sidecar_exists(self, media_path, lang):
        aliases = {
            "ja": ("ja", "jpn", "jp"),
            "id": ("id", "ind"),
            "en": ("en", "eng", "enm"),
        }.get(_normalize_lang(lang), (_normalize_lang(lang),))
        stem = os.path.splitext(media_path)[0]
        allowed = {
            os.path.basename(f"{stem}.{alias}{flag}.srt").casefold()
    ...
    def _registry(self):
        ...
        def _fn():
            rows = []
            try:
                with _registry_read_lock(self.opts["REGISTRY_FILE"]):
            ...
            for r in rows:
                ...
                stem = r.get("stem")
                if stem:
                    by_key[(stem, lang)] = dict(r, lang=lang)
                    continue
                eid = r.get("episode_id")
                if isinstance(eid, int):
                    by_key[("ep", eid, lang)] = dict(r, lang=lang)
    ...
    def _prov_breakdown(rows):
        ...
        for r in rows:
            ...
            lang = _normalize_lang(r.get("lang") or "unknown")
                ...
                out.append(
                    {
                        ...
                        "language": _normalize_lang((h.get("language") or {}).get("code2")),
                        "elapsed_s": None,
                        "ai": bool(
    ...
    def _registry_row_current(self, row, media_path, lang):
        """Validate source and target provenance without changing registry state."""
        if not isinstance(row, dict) or not isinstance(media_path, str):
            return False
        normalized = _normalize_lang(lang)
        source = row.get("source") or ""
        source_lang = _registry_source_language(source, normalized)

        def candidates(language):
            aliases = {
                "ja": ("ja", "jpn", "jp"),
                "id": ("id", "ind"),
                "en": ("en", "eng", "enm"),
            }.get(_normalize_lang(language), (_normalize_lang(language),))
            stem = os.path.splitext(media_path)[0]
            return [
        ...
        def verify(path, digest, paths):
            if not isinstance(path, str) or not isinstance(digest, str) or not path or not digest:
                return False
            try:
                if os.path.abspath(path) not in {os.path.abspath(item) for item in paths}:
                    return False
        ...
        source_paths = candidates(source_lang)
        target_paths = [
            path for path in candidates(normalized)
            if ".forced" not in os.path.basename(path).casefold()
        ]
        source_path = row.get("source_path")
        source_hash = row.get("source_hash")
        source_valid = verify(source_path, source_hash, source_paths)
        if source_valid and source == "asr" and not _has_ai_marker(source_path):
            return False
        if not source_valid:
            if row.get("source_kind") == "embedded" and ("target_path" in row or "target_hash" in row):
                target_paths = [path for path in candidates(normalized) if ".forced" not in os.path.basename(path).casefold()]
                target_path = row.get("target_path")
                if not verify(target_path, row.get("target_hash"), target_paths):
                    return False
                return source_lang == normalized or _has_ai_marker(target_path)
            if source == "asr" and not source_path and not source_hash:
                target_paths = [path for path in candidates(normalized) if ".forced" not in os.path.basename(path).casefold()]
                if "target_path" in row or "target_hash" in row:
                    target_path = row.get("target_path")
                    if not verify(target_path, row.get("target_hash"), target_paths):
                        return False
                    return _has_ai_marker(target_path)
                return any(_has_ai_marker(path) for path in target_paths if os.path.isfile(path))
            return False

        target_paths = [path for path in candidates(normalized) if ".forced" not in os.path.basename(path).casefold()]
        target_path = None
        if "target_path" in row or "target_hash" in row:
            if not verify(row.get("target_path"), row.get("target_hash"), target_paths):
                return False
            target_path = row.get("target_path")
        elif source_lang != normalized:
            target_path = next((path for path in target_paths if os.path.isfile(path)), None)
            if not target_path:
                return False
        if source_lang != normalized:
            return bool(target_path and _has_ai_marker(target_path))
        return True
    ...
    def _movies_remaining_local(self, movies=None):
        ...
        try:
            ...
            target_langs = {_normalize_lang(x) for x in str(tl).split(",") if x.strip()}
            root = self.opts.get("NAS_MEDIA_ROOT", "/mnt/nas/share/media").rstrip("/")
            entries, _latest, _ll = self._state()
            done_keys = {
                (e.get("kind") or "series", e.get("sonarrEpisodeId"), _normalize_lang(e.get("language")))
                for e in entries
            ...
            reg_by_stem = {}
            for r in reg_rows:
                stem, lang = r.get("stem"), _normalize_lang(r.get("lang"))
                if stem and lang:
            ...
            for m in (movies.get("data") or []):
                ...
                missing_codes = {
                    _normalize_lang(ms.get("code2")) for ms in (m.get("missing_subtitles") or [])
                }
                for lang in target_langs:
                    ...
                    for stem in stems:
                        ...
                        if rec:
                            break
                    if rec and self._registry_row_current(rec, mapped, lang):
                        continue
                    if self._sidecar_exists(mapped, lang):
                        continue
                    remaining += 1
                    break
            return remaining
    ...
    def _h_wanted(self, body, id=None):
        ...
        for it in data:
            ...
            seen.add(ep_id)
            missing = sorted(
                {_normalize_lang(m.get("code2")) for m in (it.get("missing_subtitles") or []) if isinstance(m, dict) and m.get("code2")}
            )
            ...
            for lang, lst in (
                ...
                if daemon.get("reachable") and lstatus not in ("done", "error"):
                    ...
                    if l_epoch is not None and now - l_epoch <= 1800:
                        lstatus = "running"
                if lang in missing and lstatus == "done":
                    lstatus = "wanted"
                lang_states[lang] = {
                    "status": lstatus,
            ...
            for lang in missing:
                if lang not in lang_states:
                    lst = latest_by_lang.get(("series", ep_id, lang)) or st or {}
                    lang_states[lang] = {
                        "status": "wanted",
                        "elapsed_s": lst.get("elapsed_s"),
                        "ts": lst.get("ts"),
                    }
            if state["status"] == "done" and any(
                lang_states[lang]["status"] == "wanted" for lang in missing
            ):
                state["status"] = "wanted"
            rf = refine.get(ep_id)
            items.append(
    ...
    def _registry_lang_entries(self, langs_by_id, eid, stem_cands, reg_by_stem):
        ...
        for stem in stem_cands:
            media_path = next(
                (stem + ext for ext in (".mkv", ".mp4", ".avi", ".ts")
            ...
            for lang, rec in (reg_by_stem.get(stem) or {}).items():
                lang = _normalize_lang(lang)
                if lang in entry or not self._registry_row_current(rec, media_path, lang):
                    continue
                entry[lang] = {
                    "status": "done",
                    "ts": rec.get("ts"),
        ...
        for eid, w in wanted_by_id.items():
            ...
            for m in (w.get("missing_subtitles") or []):
                if not isinstance(m, dict):
                    continue
                code = _normalize_lang(m.get("code2"))
                if not code:
                    continue
                if code not in entry:
                    entry[code] = {"status": "wanted", "ts": None, "elapsed_s": None}
                elif entry[code].get("status") == "done":
                    entry[code]["status"] = "wanted"
        for eid in ep_ids:
