...
class TestExternalSidecarRegistration(unittest.TestCase):
    ...
    def _patches(self, trusted=False, retime_result=None):
        retimed, stats = retime_result or (self.retimed, {"method": "order", "anchors": 0, "total": 1, "matched_frac": 0.0})
        return [
    ...
    def test_accepted_retime_registers_external_row(self):
        ...
        for p in patches:
            p.start()
        try:
            hit = o.detect_ladder_source(self.cfg, self.video, "ja", self.tmp, ep_id=42)
        finally:
            for p in patches:
                p.stop()
        self.assertEqual(hit["kind"], "jpn")
        recs = [json.loads(l) for l in open(self.registry, encoding="utf-8")]
        row = [r for r in recs if r["stem"] == self.stem and r["lang"] == "jpn"]
        self.assertEqual(len(row), 1, "exactly one jpn sidecar row expected")
        row = row[0]
        ...
        self.assertEqual(row["retimed"], "order")
    ...
    def test_registration_is_idempotent_per_content(self):
        ...
        for p in patches:
            p.start()
        try:
            o.detect_ladder_source(self.cfg, self.video, "ja", self.tmp, ep_id=42)
    ...
    def test_rejected_retime_registers_nothing(self):
        patches = self._patches(
            retime_result=(None, {"method": "order", "anchors": 0, "total": 1, "matched_frac": 0.0})
        )
        for p in patches:
            p.start()
        try:
            ...
            for p in patches:
                p.stop()
        self.assertIsNotNone(hit)  # falls through to the ASR rung
        self.assertEqual(hit["kind"], "asr")
        self.assertFalse(os.path.exists(self.registry), "no registry writes expected")
