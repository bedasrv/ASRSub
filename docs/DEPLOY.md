...
- Source language fails closed: the audio track is picked by its real tag and that code is sent to Whisper; an untagged track is probed unforced and the detected code pins the rest. If the language cannot be established the episode errors (state row `error`) and nothing is uploaded or registered `done` — a deployment never commits a subtitle whose source language is unknown. Expect no `source_stream`/`source_lang` in the registry `extra` for old rows; new rows carry both.
